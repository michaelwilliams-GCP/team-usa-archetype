...
export function ArchetypeExperience() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <section className="mx-auto grid min-h-[92vh] min-w-0 max-w-7xl gap-8 px-4 pb-10 pt-10 md:px-8 lg:grid-cols-[minmax(0,0.88fr)_minmax(440px,1.12fr)] lg:items-start">
        <div className="min-w-0 space-y-7">
          <div className="flex flex-wrap items-center gap-3">
            <div className="w-fit rounded-md border border-[color:var(--accent-solid)] bg-[var(--accent-soft)] px-3 py-2 text-sm font-black text-[var(--accent-text)]">
              Google Cloud x Team USA Hackathon
            </div>
            <div className="w-fit rounded-md border border-[color:var(--usa-blue)] bg-[var(--info-soft)] px-3 py-2 text-sm font-black text-[var(--info-text)]">
              Gemini analysis route
            </div>
          </div>

          <div>
            <p className="text-lg font-black uppercase text-[var(--accent-text)]">American performance profile engine</p>
            <h1 className="mt-3 max-w-4xl text-5xl font-black leading-[0.94] text-[var(--foreground)] sm:text-6xl lg:text-7xl">
              <span className="block text-[var(--accent-text)]">Team USA</span>
              <span className="block">Archetype Lab</span>
            </h1>
            <div className="usa-rule mt-5 h-2 w-full max-w-xl rounded-full" />
            <p className="mt-5 max-w-2xl text-lg leading-8 text-[var(--muted)]">
              A scouting-room interface for matching a fan profile against historical Team USA body metrics, medal patterns,
              and specialist AI analysis. Results are conditional mirrors, not selection or performance predictions.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={label} className="usa-card rounded-md border border-[color:var(--border)] bg-[var(--panel)] p-4 pt-5 shadow-[var(--shadow)]">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {['Coach fit', 'Biomechanics', 'Historical lineage'].map((label, index) => (
                <div key={label} className="flex items-center gap-3">
                  <span className={`flex h-9 w-9 items-center justify-center rounded-md font-black text-white ${
                    index === 1 ? 'bg-[var(--usa-blue)]' : 'bg-[var(--usa-red)]'
                  }`}>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] lg:items-start">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              <article key={title} className="usa-card rounded-lg border border-[color:var(--border)] bg-[var(--panel-strong)] p-5 pt-6">
                <div className="flex items-center gap-3">
                  <span className="h-3 w-3 rounded-full bg-[var(--usa-red)]" />
